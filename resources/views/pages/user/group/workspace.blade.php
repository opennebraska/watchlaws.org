...
@section('html_title', $group->name.' | '.$session->title.' | '.$state->name.' | '.$group->parent->name)
@push('body')
    <x-container>
        ...
        <ul class="mb-5">
            ...
            <li><a href="{{ route('group.session.show', [$group->parent, $state->abbreviation, $session]) }}" class="underline">{{ $session->title }}</a></li>
            <li>{{ $group->name }}</li>
        </ul>
